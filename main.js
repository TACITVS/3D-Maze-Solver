...
class MazeGenerator {
  ...
  isValidCell(x, y) {
    return (
      x > 0 &&
      x < this.config.MAZE_SIZE - 1 &&
      y > 0 &&
      y < this.config.MAZE_SIZE - 1
    );
  }
  ...
  createEntranceAndExit() {
    ...
    for (
      let x = this.config.MAZE_SIZE - 2;
      x > Math.floor((3 * this.config.MAZE_SIZE) / 4);
      x--
    ) {
      if (
        this.grid[this.config.MAZE_SIZE - 2][x].type ===
        this.config.CELL_TYPES.PATH
      ) {
        this.exit = { x: x, y: this.config.MAZE_SIZE - 1 };
        this.grid[this.config.MAZE_SIZE - 1][x].type =
          this.config.CELL_TYPES.PATH;
        break;
      }
    }
...
class Agent extends GameEntity {
  constructor(maze, startPosition, mazeGenerator) {
    super(startPosition);
    this.config = GameConfig.getInstance();
    this.maze = maze;
    this.trail = [];
    ...
    this.wallFollowDirection = 'right';
    this.exitLocation = mazeGenerator.exit;
  }
  ...
  update() {
    ...
    if (this.targetPosition) {
      ...
      if (distance > this.speed) {
        ...
        if (
          this.currentCell.y === this.config.MAZE_SIZE - 1 &&
          Math.abs(
            this.position.x -
              (this.maze[this.config.MAZE_SIZE - 1].findIndex(c => c.type === this.config.CELL_TYPES.PATH) - this.config.MAZE_SIZE / 2) *
...
class MazeGame {
  ...
  setup() {
    ...
    this.agent = new Agent(this.maze, startPos, this.mazeGenerator);
  }
